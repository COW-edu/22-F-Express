...
- 클라이언트-서버 프로토콜이기도 함
    - 즉 서버와 클라이언트가 웹에서 데이터를 주고받기 위한 프로토콜
- 요청과 응답이 존재

## HTTP 특징
### 비연결지향(Connectionless)
- HTTP를 통해 서버에 요청(Request)를 했을때 그에 맞는 응답(Response)를 보낸 후 연결
을 끊는다
-  비연결지향으로 연결을 끊는 순간 서버와 클라이언트의 통신이 끝나며 상태 정보는 유지되
지 않는 특징이 있다.
   - 상태정보 : CPU가 기계어 명령의 각 단계마다 현재 무엇을 하고 있는지 외부로 알려주는 신호
### 무상태(Stateless) : 쿠키 세션 토큰
- HTTP가 클라이언트를 기억하는 방법
   - Cookie
      - 사용자의 정보가 브라우저에 저장
      - 브라우저가 다르면 다른 사용자로 인식
      - 위변조 위험성 높음
   - Session
      - 사용자의 정보를 서버에 저장
         - 접속자가 많으면 서버 메모리가 차지되 무리가 될 수 있음
      - Cookie보단 안전
   - Token
      - 데이터를 토큰으로 치환하여 원본데이터 대신 사용
