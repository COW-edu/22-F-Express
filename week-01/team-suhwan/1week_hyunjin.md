...
- node.js Program Lifecycle
    - node app.js  파일 실행 → 스크립트가 시작되어 node.js가 파일 전체를 살펴보고 코드를 분석한 후 변수와 함수를 등록(전체 코드를 읽고 실행을 시작)
    - 이벤트 루프: 작업이 남아 있는 한 계속해서 작동하는 루프 프로세스, 이벤트 리스너가 있는 한 계속 작동
    
    Node.js가 이 패턴을 사용하는 이유:  단일 스레드 js를 실행하기 때문 → 실행중인 컴퓨터에서 전체 노드 프로세스가 하나의 스레드를 사용한다는 뜻
...
- 이벤트 루프
    ...
    - Timers
    타이머를 설정하면 타이머가 끝나면 실행할 함수를 Node.js가 알고있어서 항상 새로운 루프가 반복이 일어날 때 마다 시간이 다 된 콜백을 실행한다.  즉, 타이머가 끝난 콜백을 실행한다.
    - Pending Callbacks 
