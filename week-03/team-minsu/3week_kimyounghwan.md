...
**클라이언트 사이드**
간단히 말하면, 사용자 측에서 처리하는 것이다. 
클라이언트가 입력한것을 수신해서 HTML로 만들어서 크롬과 같은 어플리케이션에 해석해서 표시.    
대표적으로 자바스크립트(Javascript)이 있다.
클라이언트 사이드 언어 다루 개발자를 **프론트엔드 개발자**라고 부른다

**서버 사이드**
간단히 말하면, 서버 측에서 처리하는 것이다.
...
서버 사이드 언어를 다루는 개발자를 **백엔드 개발자**라고 부른다

**서버와 클라이언트는 웹 페이지로 서로 커뮤니케이션을 한다고 볼 수 있다**
