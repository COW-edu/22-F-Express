...
여러 조각으로 나눠서 사용하는 이유가 무엇인가요?

 하나의 함수 안에 많은 내용을 저장하면, 함수 선언부 주변 코드와 끝부분의 코드 로직까지 살펴볼 때까지, 스크롤 하며 읽거나 하나의 함수를 파악하는데 시간이 오래 걸리기에 가독성이 떨어진다. 또한, 비동기 처리 등의 로직이 복잡하거나 직관적이지 않은 경우 기억하기도 어렵다. 따라서, 하나의 함수를 사용하기보다는, 여러 블록(조각)들로 분할하여 사용하는 것이 용이하다. 문맥에 따라 함수를 분할하면 수정할 때에도 수월하다.
...
next 함수 내부 에 while 순환이 있습니다 .순환 할 때마다 stack 에서 layer 를 꺼 냅 니 다.이 layer 에는 경로 와 미들웨어 정보 가 포함 되 어 있 습 니 다.그리고 layer 와 요청 한 path 로 일치합니다. 일치 하면 layer.handle 을 실행 합 니 다.request,미들웨어 함수 호출.그러나 일치에 실패 하면 다음 layer(즉 미들웨어)를 순환합니다.
